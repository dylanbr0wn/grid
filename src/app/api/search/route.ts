...
export async function GET(request: NextRequest) {
  try {
    ...
    if (limit instanceof type.errors) {
      return new Response(JSON.stringify({ error: `Invalid limit parameter: ${limit.summary}` }), {
        status: 400,
    ...
    if (offset instanceof type.errors) {
      return new Response(JSON.stringify({ error: `Invalid offset parameter: ${offset.summary}` }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
