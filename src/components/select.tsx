	...
	return (
		<Field.Root className="flex w-full max-w-64 flex-col items-start gap-1">
			...
			<BSelect.Root value={value} items={items} onValueChange={onChange}>
				<BSelect.Trigger
					className={cn(
						'flex h-10 min-w-36 items-center justify-between gap-3 rounded-md border border-neutral-200 pr-3 pl-3.5 text-base text-neutral-300 select-none hover:bg-neutral-900 focus-visible:outline focus-visible:-outline-offset-1 focus-visible:outline-teal-400 data-[popup-open]:bg-neutral-900 cursor-default',
						className
					)}
				>
					<BSelect.Value />
				...
				<BSelect.Portal>
					...
					>
						...
						<BSelect.ScrollDownArrow className="bottom-0 z-[1] flex h-4 w-full cursor-default items-center justify-center rounded-md bg-neutral-900 text-center text-xs before:absolute before:top-[-100%] before:left-0 before:h-full before:w-full before:content-[''] data-[direction=down]:bottom-0 data-[direction=down]:before:bottom-[-100%]" />
					</BSelect.Positioner>
				</BSelect.Portal>
			</BSelect.Root>
